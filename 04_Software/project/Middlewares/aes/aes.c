...
char *encrypt(char *plain_data, char *test_key, char *test_iv)
{
	...
	uint16_t blocks = len/16+1;
	uint8_t padding = blocks*16-len;
	uint8_t str[blocks*16],key[16],iv[16];
	static char encoded_data[64];
	uint8_t password[64];
	memcpy(str, plain_data, len);
	for(uint8_t i = len;i<blocks*16;i++)
	{
		str[i] = padding;
	}
	...
	aes_init(key);
	aes_encrypt(str, password, 16, iv);
	base64_encode(password, encoded_data, 16);
	return encoded_data;
}
